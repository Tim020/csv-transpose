...
def main(
    ...
    for input_file in next(os.walk(input_path), (None, None, []))[2]:
        ...
        with open(input_file_path, "r", encoding="UTF-16", newline="") as csvfile:
            ...
            for csv_row in csvreader:
                ...
                transform_data = {
                    key.strip(): value.strip() for key, value in csv_row.items()
                }

                if col not in transform_data:
                    logger.warning(
                        f"Unable to process file as column `{col}` is missing"
                    )
                    process_file = False
                    break
                if row not in transform_data:
                    logger.warning(
                        f"Unable to process file as column `{row}` is missing"
                    )
                    process_file = False
                    break
        ...
        if process_file:
            ...
            for file_row in rows:
                if file_row[col] in file_data:
                    logger.warning(
                        f"Unable to process file as column `{file_row[col]}` appears more "
                        f"than once"
                    )
                    process_file = False
                    break
    ...
    with open(output_file, "w", encoding="UTF-16", newline="") as csvfile:
        writer = csv.DictWriter(
            csvfile, fieldnames=sorted(list(all_keys)), delimiter="\t"
        )
        writer.writeheader()
